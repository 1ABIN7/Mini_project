...
# Enable gzip compression for all JSON/text responses
Compress(app)
...
def get_jobs():
    ...
    Supports ?limit=50&offset=0 pagination and ?fields=map for slim map payloads.
    """
    try:
        ...
        slim_fields = request.args.get('fields') == 'map'

        # Clamp limit to a safe max to prevent abuse
        ...
        # Slim down payload for map-only requests
        if slim_fields:
            map_keys = {'id', 'title', 'company_name', 'latitude', 'longitude', 'wage', 'distance_km'}
            page_jobs = [{k: j[k] for k in map_keys if k in j} for j in page_jobs]

        return jsonify({"data": page_jobs, "total": total, "limit": limit, "offset": offset}), 200
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def create_job():
    ...
    try:
        ...
        try:
            ...
            # If slots column doesn't exist yet, retry without it
            if 'slots' in str(insert_err):
                print("[CREATE_JOB] 'slots' column missing — inserting without it. Run: ALTER TABLE jobs ADD COLUMN IF NOT EXISTS slots integer NOT NULL DEFAULT 1;")
                job_data.pop('slots', None)
                response = supabase.table('jobs').insert(job_data).execute()
            else:
                raise
...
def update_job(job_id):
    ...
    try:
        ...
        allowed_fields = ['title', 'description', 'location', 'wage', 'requirements', 'urgent', 'status', 'image_url', 'slots']
...
def check_email_exists():
    ...
    Uses a targeted single-row Supabase admin lookup instead of scanning all users.
    """
    try:
        data = request.json
        ...
        # Use Supabase admin v1 endpoint with filter — O(1) lookup, not a full table scan
        resp = requests.get(
            f'{supabase_url}/auth/v1/admin/users',
            headers=headers,
            params={'filter': f'email.eq.{email}', 'per_page': 1}
        )
        ...
        # Fallback: safe default — assume email doesn't exist so sign-up isn't blocked
        return jsonify({"exists": False}), 200

    except Exception as e:
        print(f"[CHECK_EMAIL] Error: {e}")
        # Fail open so sign-up flow is never blocked by this check
        return jsonify({"exists": False}), 200
...
def update_application_status():
    ...
    When accepting: checks if job slots are now filled; if so, auto-closes the
    job and notifies all remaining non-accepted applicants.
    """
    try:
        data = request.json
        employer_id = request.user.id
        ...
        # Try fetching slots from jobs; fall back gracefully if column doesn't exist yet
        try:
            app_res = supabase.table('applications').select('*, jobs!inner(employer_id, title, slots, status)').eq('id', application_id).execute()
        except Exception:
            app_res = supabase.table('applications').select('*, jobs!inner(employer_id, title, status)').eq('id', application_id).execute()
            if app_res.data:
                app_res.data[0]['jobs']['slots'] = 1  # default until column is added
        ...
        if app_data['jobs']['employer_id'] != employer_id:
            return jsonify({"error": "Forbidden", "message": "You do not own this job"}), 403

        job_id = app_data['job_id']
        job_title = app_data['jobs'].get('title', 'this job')
        slots = int(app_data['jobs'].get('slots') or 1)
        ...
        # --- AUTO-CLOSE LOGIC ---
        # Only trigger when accepting and the job is still open
        auto_closed = False
        if new_status == 'accepted' and app_data['jobs'].get('status') == 'open':
            # Count total accepted applications for this job (include this one)
            accepted_res = supabase.table('applications') \
                .select('id') \
                .eq('job_id', job_id) \
                .eq('status', 'accepted') \
                .execute()
            accepted_count = len(accepted_res.data or [])
            ...
            if accepted_count >= slots:
                ...
                # Notify + reject all remaining pending applicants
                pending_res = supabase.table('applications') \
                ...
                for pending_app in (pending_res.data or []):
                    sid = pending_app['student_id']
                    # Mark as rejected so they don't linger as pending
                    supabase.table('applications').update({'status': 'rejected'}).eq('id', pending_app['id']).execute()
                    # In-app notification
                    create_notification(
                        sid,
                        f"The position for '{job_title}' has been filled. Thank you for applying — keep an eye out for new opportunities!",
                        "status_update"
                    )
                    # Email notification
                    applicant_email = get_user_email(sid)
                    send_email(
                        applicant_email,
                        f"Update on your application: {job_title}",
                        f"Hi there,\n\nThank you for applying for '{job_title}' on AfterBell.\n\nUnfortunately, all positions for this role have now been filled and your application has been closed.\n\nDon't be discouraged — new jobs are posted every day. Log in to discover more opportunities near you!\n\nBest wishes,\nThe AfterBell Team"
                    )

                print(f"[AUTO-CLOSE] Job '{job_title}' (id={job_id}) auto-closed after {accepted_count}/{slots} slots filled. {len(pending_res.data or [])} pending applicants notified.")

        return jsonify({
            "data": update_res.data[0],
            "message": f"Application {new_status} successfully",
            "job_auto_closed": auto_closed
        }), 200
    except Exception as e:
        import traceback
        traceback.print_exc()
...
@require_auth
def submit_rating():
    """Submit a bidirectional rating (Student -> Employer or Employer -> Student)"""
    try:
        data = request.json
        reviewer_id = request.user.id
        target_id = data.get('target_id')
        job_id = data.get('job_id')
        rating_score = max(1, min(5, int(data.get('rating', 0))))
        comment = data.get('comment', '')

        if not target_id or not job_id or rating_score == 0:
            return jsonify({"error": "Missing target_id, job_id, or rating"}), 400
        ...
        return jsonify({"message": "Rating submitted successfully", "data": res.data[0]}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/users/<user_id>/ratings', methods=['GET'])
def get_user_ratings(user_id):
    """Get the aggregated rating score and reviews for a user"""
    try:
        res = supabase.table('ratings').select('*, profiles!ratings_reviewer_id_fkey(full_name)').eq('target_id', user_id).order('created_at', desc=True).execute()
        ratings = res.data or []
        if not ratings:
